...
/**
 * Validates that the time segment doesn't exceed the maximum length requirement.
 * Only validates if both time fields are valid and error-free.
 */
export class SegmentMaxLengthValidator extends ValidationHandler {
  constructor() {
    super();
  }
  protected override handle(context: ValidationConfig): void {
    const { formData, timeStampRegex, maxSegmentLength } = context;
    ...
    if (
      ...
      if (noteSegmentLength > maxSegmentLength) {
        this.errors['endTime'] = `Segment must be ≤${maxSegmentLength}s. Adjust start/end.`;
      }
    }
  }
}
